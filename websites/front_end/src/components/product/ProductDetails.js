                                                                ...
                                                                return (
                                                                    <>
                                                                        <div className="variant-section ">
                                                                            <div className={`variant-element ${variant_index === index ? 'active' : ''}  ${Number(productdata.is_unlimited_stock) ? "" : (variant.cart_count >= variant.stock ? "out_of_stock" : "")} `} key={index}>
                                                                                <label className="element_container " htmlFor={`variant${index}`}>
                                                                                    <div className="top-section">

                                                                                        <input type="radio" name="variant" id={`variant${index}`} checked={variant_index === index} disabled={Number(productdata.is_unlimited_stock) ? false : (variant.cart_count >= variant.stock ? true : false)} onChange={() => handleVariantChange(variant, index)} />
                                                                                    </div>
                                                                                    <div className="h-100">
